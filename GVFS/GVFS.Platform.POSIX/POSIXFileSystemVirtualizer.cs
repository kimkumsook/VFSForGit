...
{
    public abstract class POSIXFileSystemVirtualizer : FileSystemVirtualizer
    {
        ...
        private const string ClassName = nameof(POSIXFileSystemVirtualizer);

        private VirtualizationInstance virtualizationInstance;

        public POSIXFileSystemVirtualizer(GVFSContext context, GVFSGitObjects gitObjects)
            : this(context, gitObjects, virtualizationInstance: null)
        {
        }

        protected POSIXFileSystemVirtualizer(
            GVFSContext context,
            GVFSGitObjects gitObjects,
            VirtualizationInstance virtualizationInstance)
            : base(context, gitObjects)
        {
            if (virtualizationInstance == null)
            {
                throw new ArgumentNullException(nameof(virtualizationInstance));
            }

            this.virtualizationInstance = virtualizationInstance;
        }
        ...
        public override FileSystemResult WritePlaceholderFile(
            ...
            string sha)
        {
            // TODO(POSIX): Add functional tests that validate file mode is set correctly
            ...
            out UpdateFailureReason failureReason)
        {
            ...
            // TODO(POSIX): Add functional tests that include:
        ...
        protected override bool TryStart(out string error)
        {
            ...
            {
                this.Context.Tracer.RelatedError($"{nameof(this.virtualizationInstance.StartVirtualizationInstance)} failed: {result:X}({result:G})");
            ...
            try
            {
                if (!this.GitObjects.TryCopyBlobContentStream(
                    ...
                    (stream, blobLength) =>
                    {
                        ...
                        // TODO(POSIX): Find a better solution than reading from the stream one byte at at time
            ...
            IntPtr fileHandle)
        {
            try
            {
                ...
                if (placeholderVersion != FileSystemVirtualizer.PlaceholderVersion)
                {
                    ...
                    // TODO(POSIX): Is this the correct Result to return?
                    ...
                    if (!this.GitObjects.TryCopyBlobContentStream(
                        ...
                        (stream, blobLength) =>
                        {
                            ...
                            while (nextByte != -1)
                            {
                                ...
                                if (result != Result.Success)
                                {
                                    activity.RelatedError(metadata, $"{nameof(this.virtualizationInstance.WriteFileContents)} failed, error: {result:X}({result:G})");
        ...
        {
            this.Context.Tracer.RelatedError($"{nameof(POSIXFileSystemVirtualizer)}::{nameof(this.OnLogError)}: {errorMessage}");
        }
        ...
        private void OnFileRenamed(string relativeDestinationPath, bool isDirectory)
        {
            ...
            // On Linux, ProjFS already provides the relativeSourcePath should it be required.
            this.OnFileRenamed(
            ...
            string triggeringProcessName)
        {
            try
            {
                ...
                catch (SizesUnavailableException e)
                {
                    ...
                    metadata.Add(nameof(result), "{result:X}({result:G})");
